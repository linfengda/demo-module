package com.lfd.soa.srv.demo.ctrl;

import com.lfd.soa.common.bean.Result;
import com.lfd.soa.common.util.JsonUtil;
import com.lfd.soa.srv.demo.bean.req.BomMaterialQueryReq;
import com.lfd.soa.srv.demo.support.apivalidator.annotation.ApiValidator;
import lombok.extern.slf4j.Slf4j;
import org.springframework.web.bind.annotation.*;

import javax.validation.constraints.NotBlank;

/**
 * 模拟plm系统对外api
 * @author linfengda
 * @date 2020-09-28 00:08
 */
@Slf4j
@RestController()
@RequestMapping("/plm/outer")
public class PlmOuterController {

    @PostMapping("/getOrderBigBomMaterials")
    public Result getOrderBigBomMaterials(@ApiValidator @RequestBody BomMaterialQueryReq bomMaterialQueryReq) {
        log.info("查询plm系统订单大货物料信息，bigBomMaterialQueryDTO={}", JsonUtil.toJson(bomMaterialQueryReq));
        return new Result("请求成功");
    }

    @GetMapping("/getMaterialPrice")
    public Result getMaterialPrice(@NotBlank @RequestParam String sku) {
        log.info("查询plm系统物料单价信息，sku={}", sku);
        return new Result("请求成功");
    }
}
